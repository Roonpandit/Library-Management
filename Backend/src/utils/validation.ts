  ...
  export const bookSchema = Joi.object({
    ...
    ISBN: Joi.string().pattern(/^(?:\d{10}|\d{13})$/).required().messages({
    ...
    if (value.genre === 'Academic' && value.copiesAvailable < 5) {
      return helpers.error('custom.academicCopies', { message: 'Academic books must have at least 5 copies available' });
    }
    return value;
  });
  ...
  export function validate<T>(data: any, schema: Joi.ObjectSchema): ValidationResult<T> {
